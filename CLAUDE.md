...
**Known gaps / next up:** Wayland input beyond keyboard — pointer + touch decoding into `Wl_Input_Event`s (keyboard works incl. shift/caps modifier-level keysyms). Explicit fence sync (currently `glFinish()` — coarse but correct). Deeper Vulkan examples beyond the triangle DMA-BUF smoke. Server-allocated object IDs (0xFF000000+ range). Fractional scaling. (Phase 6 Wayland backend for vendored Simp/Input/Window_Creation is DONE — see the "Wayland/X11 backend layer" section.)
...
- `modules/Input/{x11,wayland}.jai` — `update_window_events` (x11.jai) branches on `running_wayland()` to `update_wayland_window_events` (wayland.jai), which drains `WS.provisional_input_events` (decoded by the pump) into `events_this_frame` / `input_button_states` — reusing x11.jai's `get_key_code` (xkb keysyms share X11 keysym numbering) — and emits `Window_Resize_Record`s from the pump's `size_dirty` flag (the Wayland equivalent of X11 ConfigureNotify).

The point: the vendored Simp/Input/Window_Creation run unchanged-in-shape on Wayland, picking the backend by value (`display_manager` / `running_wayland`) rather than inline `#if`. **Upstream-unmodified `invaders` runs correctly on Wayland** — render-target ownership, frame-callback pacing, keyboard input, compositor-driven resize — with ldd still libc-only.
