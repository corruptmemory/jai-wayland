...
**Known gaps / next up:** Wayland **drag-and-drop** — `wl_data_device` (the XDND analogue); `X11.enable_drag_and_drop` no-ops on Wayland with a logged warning, so files can't be dropped onto a window yet (the GetRect examples otherwise run). Wayland input niceties — AltGr / level-3 keysyms (keyboard does Shift/Caps levels), `wp_relative_pointer_v1` for edge-crossing mouse-look (pointer currently uses surface-position deltas), touch is structural/untested (no touch device on the dev machine). Explicit fence sync (currently `glFinish()` — coarse but correct). Deeper Vulkan examples beyond the triangle DMA-BUF smoke. Server-allocated object IDs (0xFF000000+ range). Fractional scaling. (Phase 6 Wayland backend for vendored Simp/Input/Window_Creation — render + full input (keyboard/pointer/touch) + resize — is DONE; `invaders` (2D) and `skeletal-animation` (3D+UI) run on it. See the "Wayland/X11 backend layer" section.)
...
**Upstream vendored modules** (`modules/Window_Creation/`, `modules/Simp/`, `modules/GetRect/`, `modules/GetRect_LeftHanded/`) — Jai distribution modules copied verbatim from `~/jai/jai/modules/` for compatibility with upstream graphical examples. Two minimal patch sites: our `modules/X11/module.jai` self-initializes via `init_x11`/`init_glx` in `init_global_display`; `modules/Simp/backend/gl.jai` uses our `gl_load(glXGetProcAddress)` signature instead of stock `gl_load(*gl)`. (Image/audio go through the **upstream** `stb_*`/`Sound_Player` modules unmodified — see "Image/audio modules" above.) The `./build.sh - invaders` and `./build.sh - skeletal_animation` targets compile upstream's `~/jai/jai/examples/{invaders,skeletal-animation}/source/...` against these vendored modules.
...
- `modules/Input/{x11,wayland}.jai` — `update_window_events` (x11.jai) branches on `running_wayland()` to `update_wayland_window_events` (wayland.jai), which drains `WS.provisional_input_events` (decoded by the pump) into Input's model, reusing x11.jai's `get_key_code` (xkb keysyms share X11 keysym numbering). Full input surface: **keyboard** — modifier-aware keysyms (Shift/Caps xkb level selection, with a level-0 fallback so one-level keys like Shift/F-keys still resolve when the modifiers event precedes the key event) + TEXT_INPUT events for printable chars; **pointer** — motion→`mouse_delta`, buttons→`input_button_states` (evdev→`Key_Code`), wheel→`MOUSE_WHEEL`+`mouse_delta_z`, enter/leave→`input_application_has_focus`; **touch** — structural (`Event.TOUCH` + `touch_pointers`; untested, no touch device on the dev machine). Also emits `Window_Resize_Record`s from the pump's `size_dirty` (the X11-ConfigureNotify equivalent) and an `Event.QUIT` on `xdg_toplevel.close`. `Window_Creation.get_mouse_pointer_position` has a `Window_Type` overload reading the pump-tracked surface position (GetRect UI hit-testing). The BO-backed FBO slots carry a depth renderbuffer (`GL_DEPTH_COMPONENT24`) for 3D apps; the vendored `modules/GL` gained the symbols those need (UBO trio, `glDrawElements`, `glVertexAttribIPointer`, renderbuffers).

The point: the vendored Simp/Input/Window_Creation run unchanged-in-shape on Wayland, picking the backend by value (`display_manager` / `running_wayland`) rather than inline `#if`. **Four upstream-unmodified graphical examples run correctly on Wayland**: `invaders` (2D game — render-target ownership, frame-callback pacing, keyboard, compositor-driven resize), `skeletal-animation` (3D skinned-mesh viewer — depth-tested mesh, GetRect UI driven by the pointer, camera by keyboard incl. Shift speed-boost, Alt+F4 / close-button quit), and the `GetRect` + `GetRect_LeftHanded` widget showcases (color pickers, dropdowns, auto-complete text-input, lists, draggable subwindows — the LEFT_HANDED variant confirms `set_buffer_transform(FLIPPED_180)` is coordinate-system-agnostic). None links any display-server/GPU lib (libm + invaders' libasound only). The one Wayland gap these surfaced: `X11.enable_drag_and_drop` is a logged no-op on Wayland (needs `wl_data_device`).
...
4. **Vendored `Window_Creation`, `Simp`, `GetRect`, `GetRect_LeftHanded` on Wayland — DONE.** The `Wayland_Support` module (the X11-mirror shared-globals layer + single event pump) carries the vendored stack on Wayland: `display_manager`/`running_wayland` value-level backend selection (the old `context.simp_dispatch` indirection is gone), the `wayland_global_windows` registry, GL-on-GBM + DMA-BUF present (vertically flipped via `set_buffer_transform`), compositor-driven resize via `Window_Resize_Record`s, full input (keyboard incl. Shift/Caps modifier keysyms + TEXT_INPUT, pointer motion/buttons/wheel + focus, structural touch) decoded into stable `Wl_Input_Event`s, depth-buffered FBO slots for 3D, and `.QUIT`/Alt+F4/close-button quit. **Two upstream-unmodified examples run correctly on Wayland — `invaders` (2D) and `skeletal-animation` (3D + GetRect UI) — with no display-server/GPU libs linked.** Niceties left: AltGr/level-3 keysyms, `wp_relative_pointer_v1` (edge-crossing mouse-look), touch runtime validation (no device on dev machine). The eventual "raylib-light with Jai Simp+GetRect flavor" layer wraps this (keeps the low-level API unchanged, doesn't replace it). See the "Wayland/X11 backend layer" section above for the full architecture.
