...
4. **Vendored `Window_Creation`, `Simp`, `GetRect`, `GetRect_LeftHanded`** with Linux runtime backend selection between Wayland and X11. Phase 6 in progress on the `upstream-integration` branch: upstream's invaders example is now playable against our vendored stack via `./build.sh - invaders` (X11 backend, ldd-clean — sound, keyboard, animation all confirmed). **Wayland backend dispatch within Window_Creation is the remaining ~70% of the work and the design question is open** — see `docs/plans/2026-05-26-wayland-backend-question.md`. The load-bearing decision is how GL actually works on Wayland under our "no libwayland linkage" thesis (same fundamental incompatibility we hit with `VK_KHR_wayland_surface`). The eventual "raylib-light with Jai Simp+GetRect flavor" goal is framed against these existing Jai module names; keeps the low-level API unchanged, the layer wraps doesn't replace.
