...
**Backend dispatch infrastructure** (Stage 1 of Phase 6, lives alongside the vendored upstream modules):
- `modules/Window_Type.jai` — vendored from upstream with the Linux branch rewritten as a tagged union (`union wtype: Window_Tag { .X11 ,, x11: X11.Window; .Wayland ,, wayland: Wayland_Window_State }`). `Wayland_Window_State` is a placeholder struct in Stage 1; Stage 2 will populate it with EGL context + `wl_surface`/`xdg_surface` + BO slot state. Provides `operator ==` overloads for Window_Type-to-Window_Type and Window_Type-to-X11.Window comparisons; `Window_Creation/module.jai` re-exports these via `operator == :: WT.operator==;` so consumers (invaders, Simp, Input) can compare values without an explicit Window_Type import.
- `modules/Simp/backend/dispatch.jai` — defines `Simp_Op_Args` (tagged union of per-op argument payloads), `Simp_Backend_Dispatch` function-pointer type, and `#add_context simp_dispatch: Simp_Backend_Dispatch;`. The dispatch shape is op-tagged so adding a new backend op doesn't break the function-pointer ABI.
- `modules/Simp/backend/x11_dispatch.jai` — `simp_x11_dispatch` implementation; the X11/GLX-specific paths formerly inline-`#if`'d in `backend/gl.jai` are relocated here. Anonymous-imports X11 + uses the runtime-loaded GLX entry points.
- `modules/Simp/backend/wayland_dispatch.jai` — Stage 2 stub. Currently asserts and exits; Stage 2 fills it in.
- `modules/Window_Creation/linux_init.jai` — new file (not vendored). `init_linux_window()` detects the runtime environment (Stage 1: hardcoded X11) and pushes the appropriate `simp_dispatch` into context. Called lazily on first `create_window` invocation so applications that never create a window pay zero context-stack cost.
- `modules/Input/` — vendored from upstream (Phase 6 Stage 1 Task 6B). The Linux `x11.jai` is patched to construct `Window_Type` values explicitly via `.{ wtype = .X11, x11 = hwnd }` — required by the tagged-union shape since the upstream code assumed `Window_Type` was a scalar `X11.Window` alias and assigned directly.

The point of this layer is to keep `modules/Simp/backend/gl.jai` and its callers free of inline `#if OS == .LINUX` branches that distinguish X11 from Wayland. Backend choice happens once at `create_window` time and is then ambient via context. Stage 2 replaces the `running_wayland()` hardcode with real detection and fills in `simp_wayland_dispatch`.
...
4. **Vendored `Window_Creation`, `Simp`, `GetRect`, `GetRect_LeftHanded`** with Linux runtime backend selection between Wayland and X11. **Phase 6 Stage 1 complete on `upstream-integration`: context-based backend dispatch wired through vendored Simp; invaders runs identically through `context.simp_dispatch` instead of inline `#if OS == .LINUX` branches in Simp; ldd-clean preserved.** Stage 2 (Wayland dispatcher implementation) is the remaining work — see `docs/plans/2026-05-26-context-dispatch-stage1-design.md` for the architecture this established and `docs/plans/2026-05-26-wayland-backend-question.md` for the Stage-2 Layer-3 decision. The load-bearing decision is how GL actually works on Wayland under our "no libwayland linkage" thesis (same fundamental incompatibility we hit with `VK_KHR_wayland_surface`). The eventual "raylib-light with Jai Simp+GetRect flavor" goal is framed against these existing Jai module names; keeps the low-level API unchanged, the layer wraps doesn't replace.
