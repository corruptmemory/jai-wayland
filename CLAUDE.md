...
**Current status:** Phases 1-4 complete, demarshal + screen discovery + dynamic resize done. Working Wayland client — connects to compositor, discovers globals and outputs, displays resizable windows via wl_shm with pooled buffers. No libwayland dependency.
...
./build.sh - demarshal_test  # Build and run 12 demarshal macro tests
...
- `demarshal.jai` — compile-time demarshal macro (mirror of marshal). `demarshal(*args, payload, recv)` fills a single event args struct; `demarshal_event(*event, opcode, payload, recv)` dispatches opcode into a tagged union. Walks type_info at compile time, emits type-specific reads. Handles all 8 wire types: u32, s32, Fixed, string, `[] u8`, Fd (via `receive_pop_fd`), `*Interface` (`New(T,, temp)`), `*void`. For tagged unions, walks `tagged_union_bindings` to generate opcode→variant dispatch.

**Generated module** (`modules/wayland/`):
- `module.jai` → `#load` chain → `types.jai`, `wire.jai`, `connection.jai`, `marshal.jai`, `demarshal.jai`, `shm.jai`, `registry.jai`, `session.jai`, `output.jai` → per-protocol directories → per-interface files
...
- `output.jai` — `Mode_Info`, `Screen_Info` structs, `get_screens_info()` discovers all compositor outputs via wl_output bind + sync roundtrip + `demarshal_event`. Returns `[] Screen_Info` with name, modes, current_mode, scale_factor, geometry. First real consumer of demarshal.
- **Design: no inversion of control.** Application owns the event loop via `for session() { ... }`, switches on object IDs, decodes events with `demarshal`/`demarshal_event` or inline `read_u32`/`read_string`. No callbacks, no dispatch tables, no event queues.
...
- `hello_window.jai` — ~150 lines, discovers native screen resolution via `get_screens_info()`, allocates one shm pool sized to it, creates surface/xdg_surface/toplevel, handles `xdg_toplevel.configure` for dynamic resize by minting new `wl_buffer` from the same pool. Dark blue gradient, no remapping on resize.
...
- `tests/demarshal_test.jai`: 12 tests (round-trip fixed/string/array/Fixed/fd/empty, *void, *Interface temp alloc, tagged union dispatch with real Wl_Output_Event)
...
- Compile-time code generation: `marshal`/`demarshal` macros use `#expand` + `#insert #run` to walk struct type_info and emit serialization/deserialization code. `demarshal_event` walks `tagged_union_bindings` for opcode dispatch. Debug expansions in `.build/.added_strings_wN.jai`
...
3. **Server-allocated objects** — Handle `new_id` args in events (e.g., wl_data_device.data_offer, tablet hotplug). IDs from 0xFF000000+ range.
4. **Double buffering** — Front/back buffers in the same shm pool for tear-free resizes. Swap on `wl_buffer.release` event.
