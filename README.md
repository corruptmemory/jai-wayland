...
- **Asynchronous, triple-buffered present** — `wl_present_and_pace` paces to vsync via the *previous* frame's `wl_surface.frame` callback: a CPU-bound app never blocks (the prior callback already fired → no per-frame latency), a fast app caps to vsync (no uncapped GPU burn) — the same shape as GLX's triple-buffered swap. 3 BO slots give the render-ahead headroom. This replaced an earlier synchronous block-on-the-frame-callback present that cost ~3.3 ms/frame; dragging now matches XWayland's feel. The wait is also **bounded** (50 ms): a window the compositor isn't presenting (a hidden tab in a Hyprland group, another workspace) sends no frame callbacks, so it's flagged starved and skipped rather than stalling the single-threaded render loop — a visible sibling keeps full vsync pacing, an all-occluded app idles. (Profiling found the present path was *not* the frame-rate bottleneck — the ~28.5 ms/frame backend-agnostic GetRect+Simp CPU work caps both backends at ~30–34 fps. See `docs/plans/2026-05-31-wayland-async-present.md`.)
...
- **Explicit fence sync** — the present path still does a coarse `glFinish()` (~0.68 ms/frame) before handing buffers to the compositor; `Gl_Slot` has placeholder fence fd fields for future `EGL_KHR_fence_sync` / `wp_linux_drm_syncobj_v1` work. (The *bigger* present cost — a synchronous frame-callback block, ~3.3 ms/frame — is **gone** as of 2026-05-31: `wl_present_and_pace` is now an asynchronous, triple-buffered, vsync-paced present that throttles on the *previous* frame's callback, dropping present overhead ~4 ms→~0.7 ms and matching XWayland's drag feel. See `docs/plans/2026-05-31-wayland-async-present.md`. The real ~30–34 fps ceiling is the backend-agnostic ~28.5 ms/frame GetRect+Simp CPU cost — identical on X11 and Wayland — unchanged by this.)
