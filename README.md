...
- **Selectable triple-buffered present** — `wl_present_and_pace` supports `JAI_WAYLAND_PRESENT_MODE=fifo|mailbox|immediate`. `fifo` is the default: it paces to vsync via the *previous* frame's `wl_surface.frame` callback, with a bounded wait (50 ms) so occluded windows do not stall visible siblings. `mailbox` skips the frame-callback wait, commits when a spare BO slot exists, and drops rendered frames when the queue is full so the app loop is not callback-paced. `immediate` uses the same low-latency queue policy and requests `tearing-control-v1` async presentation when the compositor advertises it; the compositor may still ignore that hint. Low-latency modes actively zero-timeout poll the Wayland socket because they no longer have FIFO's blocking frame-callback receive to drain input, pings, and buffer releases. The `pacing-experiments` branch adds a small active-resize sleep for mailbox/immediate while configure events are streaming; set `JAI_WAYLAND_TRACE_RESIZE=1` to log configure, resize, BO-realloc, and resize-pacing events.
...
- **Explicit fence sync** — the present path still does a coarse `glFinish()` (~0.68 ms/frame) before handing buffers to the compositor; `Gl_Slot` has placeholder fence fd fields for future `EGL_KHR_fence_sync` / `wp_linux_drm_syncobj_v1` work. (The *bigger* present cost — a synchronous frame-callback block, ~3.3 ms/frame — is **gone** as of 2026-05-31. The default `fifo` mode now throttles on the previous frame callback; opt-in `mailbox` / `immediate` modes remove that callback gate and use buffer-release backpressure instead. See `docs/plans/2026-05-31-wayland-async-present.md`.)
- **Low-latency resize pacing experiment** — `docs/plans/2026-06-02-wayland-low-latency-resize-pacing.md` records the open Hyprland/GetRect resize-pacing work on the `pacing-experiments` branch. The current experiment keeps app-visible resize immediate and adds only a tiny active-resize sleep in mailbox/immediate mode.
