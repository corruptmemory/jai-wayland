...
- **Bucketed GPU slot capacity** — `docs/plans/2026-06-02-wayland-low-latency-resize-pacing.md` records the active resize-churn work: decouple each window's logical size from its BO/EGLImage/GL/wl_buffer allocation size, grow slots in capacity buckets, recreate only when the logical size exceeds capacity, and crop the oversized buffer with `glViewport`/`glScissor` + `wp_viewporter`. (Supersedes an abandoned active-resize-sleep experiment.)
